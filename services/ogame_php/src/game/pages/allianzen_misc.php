...
function PageAlly_Leave ()
{
    ...
    if ( method () === "POST" )
    {
        if ( $_GET['weiter'] == 1) 
        {
            ...
            {
                $user = dbarray ($result);
                SendMessage ($user, $user['player_id'], va("Альянс [#1]", $ally['tag']), "Общее сообщение", va("Игрок #1 покинул альянс.", $leaver['oname']), 0);
            }
...
function PageAlly_Dismiss ()
{
    ...
    if ( method() === "POST" && $_GET['a'] == 12 && $_GET['weiter'] == 1)
    {
        ...
        else
        {
            ...
            {
                $user = dbarray ($result);
                SendMessage ($user, $user['player_id'], $from, $subj, $text, 0);
            }
...
function AllyPage_Takeover ()
{
    ...
    if ( $_GET['a'] == 18 && $_REQUEST['s'] == 1)
    {
        ...
        else
        {
            ...
            {
                $user = dbarray ($result);
                if ( $user['player_id'] != $ally['owner_id'] ) SendMessage ($user, $user['player_id'], $from, $subj, $text, 0);
            }
