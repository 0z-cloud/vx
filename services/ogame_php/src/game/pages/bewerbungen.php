...
if ( method () === "POST" )
{
    if ( $_POST['aktion'] === "Принять" && $show > 0 )
    {
        ...
        {
            $user = dbarray ($result);
            SendMessage ($user, $user['player_id'], va("Альянс [#1]", $ally['tag']), "Общее сообщение", va("Игрок #1 был принят в наш альянс.", $newcomer['oname']), 0);
        }
        SendMessage ($user, $player_id, va("Альянс [#1]", $ally['tag']), va("Регистрация [#1] принята", $ally['tag']), va("Сердечно поздравляем, Вы теперь член альянса [#1]", $ally['tag']), 0 );
    ...
    if ( $_POST['aktion'] === "Отклонить" && $show > 0 )
    {
        ...
        SendMessage ($user, $app['player_id'], va("Альянс [#1]", $ally['tag']), va("Регистрация [#1] отклонена", $ally['tag']), $reason, 0 );
    }
}
