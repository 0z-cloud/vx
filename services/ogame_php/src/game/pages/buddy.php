...
if ( key_exists ('action', $_GET) && $_GET['action'] == 1 && $_GET['buddy_id'])    // Добавить свою заявку.
{
    ...
    if ($from != $to)
    {
        ...
        else SendMessage ($user, $to, $GlobalUser['oname'], loca("BUDDY_REQUEST"), $_POST['text'], 0 );
    }
    Buddy_Home ();
}
else if ( key_exists ('action', $_GET) && $_GET['action'] == 2 && $_GET['buddy_id'])    // Принять запрос
{
    ...
    SendMessage ($user, $buddy['request_from'], loca("BUDDY_LIST"), loca("BUDDY_CONFIRM"), va(loca("BUDDY_MSG_ADDED"), $GlobalUser['oname']), 0);
    Buddy_Income ();
}
else if ( key_exists ('action', $_GET) && $_GET['action'] == 3 && $_GET['buddy_id'])    // Отклонить запрос
{
    ...
    SendMessage ($user, $buddy['request_from'], loca("BUDDY_LIST"), loca("BUDDY_REQUEST"), va(loca("BUDDY_MSG_DECLINED"), $GlobalUser['oname']), 0);
    Buddy_Income ();
}
else if ( key_exists ('action', $_GET) && $_GET['action'] == 4 && $_GET['buddy_id'])    // Отозвать свой запрос.
{
    ...
    {
        RemoveBuddy ($buddy_id);
        SendMessage ($user, $buddy['request_to'], loca("BUDDY_LIST"), loca("BUDDY_REQUEST"), va (loca("BUDDY_MSG_RECALLED"), $GlobalUser['oname']), 0 );
    }
    Buddy_Outcome ();
}
...
else if ( key_exists ('action', $_GET) && $_GET['action'] == 8 && $_GET['buddy_id'])    // Удалить из списка
{
    ...
    {
        RemoveBuddy ($buddy_id);
        SendMessage ($user, $buddy['request_to'], loca("BUDDY_LIST"), loca("BUDDY_CONFIRM"), va (loca("BUDDY_MSG_DELETED"), $GlobalUser['oname']), 0 );
    }
    if ($buddy['request_to'] == $GlobalUser['player_id'] )
    {
        RemoveBuddy ($buddy_id);
        SendMessage ($user, $buddy['request_from'], loca("BUDDY_LIST"), loca("BUDDY_CONFIRM"), va (loca("BUDDY_MSG_DELETED"), $GlobalUser['oname']), 0 );
    }
    Buddy_Home ();
}
else Buddy_Home ();
