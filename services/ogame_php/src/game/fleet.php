...
function TransportArrive ($queue, $fleet_obj, $fleet, $origin, $target)
{
    ...
    $text = "Ваш флот достигает планеты (\n" .
                ...
                "<br/>\n";
    SendMessage ($user, $fleet_obj['owner_id'], "Командование флотом", "Достижение планеты", $text, 5, $queue['end']);
    ...
    if ( $origin['owner_id'] != $target['owner_id'] )
    {
        ...
        $text = "Чужой флот игрока ".$user['oname']." доставляет на Вашу планету ".$target['name']."\n" .
                    ...
                    "<br/>\n";
        SendMessage ($user, $target['owner_id'], "Наблюдение", "Чужой флот доставляет сырьё", $text, 5, $queue['end']);
    }
}
...
function CommonReturn ($queue, $fleet_obj, $fleet, $origin, $target)
{
    ...
    SendMessage ($user, $fleet_obj['owner_id'], "Командование флотом", "Возвращение флота", $text, 5, $queue['end']);
}
...
function DeployArrive ($queue, $fleet_obj, $fleet, $origin, $target)
{
    ...
    $text = "\nОдин из Ваших флотов (".FleetList($fleet).") достиг ".$target['name']."\n" .
               ...
               "<br/>\n";
    SendMessage ($user, $fleet_obj['owner_id'], "Командование флотом", "Удержание флота", $text, 5, $queue['end']);
}
...
*/
    ...
    SendMessage ($user, $fleet_obj['owner_id'], "Командование флотом", $subj, $report, 1, $queue['end']);
    ...
    $text = "\nЧужой флот с планеты ".$origin['name']."\n" .
                ...
                "</td>\n";
    SendMessage ($user, $target['owner_id'], "Наблюдение", "Шпионаж", $text, 5, $queue['end']);
...
function ColonizationArrive ($queue, $fleet_obj, $fleet, $origin, $target)
{
    ...
    SendMessage ($user, $fleet_obj['owner_id'], "Поселенцы", "Доклад поселенцев", $text, 5, $queue['end']);
}
...
function RecycleArrive ($queue, $fleet_obj, $fleet, $origin, $target)
{
    ...
    SendMessage ($user, $fleet_obj['owner_id'], "Флот ", $subj, $report, 5, $queue['end']);
}
...
function AddUnionMember ($union_id, $name)
{
    ...
    SendMessage ($user, $user['player_id'], $GlobalUser['oname'], "Приглашение к совместной атаке", $text, 5 );
