...
function GravitonAttack ($fleet_obj, $fleet, $when)
{
    ...
    $subj_g_to_t = $subj;
    SendMessage ($user, $origin['owner_id'], "Командование флотом", "Лунная атака", $atext, 5);
    SendMessage ($user, $target['owner_id'], "Командование флотом", "Лунные толчки", $dtext, 5);
...
function StartBattle ( $fleet_id, $planet_id, $when )
{
    ...
    foreach ( $d as $i=>$user )        // Обороняющиеся
    {
        ...
        $bericht = SendMessage ($user, $user['player_id'], "Командование флотом", "Боевой доклад", $text, 6, $when );
        ...
        SendMessage ($user, $user['player_id'], "Командование флотом", $subj, "", 2, $when );
...
function RocketAttack ( $fleet_id, $planet_id )
{
    ...
    SendMessage ($user, $target_user['player_id'], "Командование флотом", "Ракетная атака", $text, 2);
}
