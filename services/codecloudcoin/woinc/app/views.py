...
from .models import HyperVisor, User, Domain, Record, Server, History, Anonymous, Setting, DomainSetting
...
#### CUSTOM SECTION
@app.route('/hypervisor/server/create', methods=['GET', 'POST'])
@login_required
def hypervisor_server_create():
    if request.method == 'GET':
        return render_template('hypervisor_server_create.html')

    if request.method == 'POST':

        fdata = request.form

        hypervisor = HyperVisor(hv_server_name=fdata['hv_server_name'], hv_server_ip=fdata['hv_server_ip'], hv_server_api_user=fdata['hv_server_api_user'], hv_server_api_password=fdata['hv_server_api_password'])

        ip_address_validation = fdata['hv_server_ip']
        parts = ip_address_validation.split(".")
        if len(parts) != 4:
            return render_template('hypervisor_server_create.html', hypervisor=hypervisor, hypervisor_ip_check=True)
        for item in parts:
            if not 0 <= int(item) <= 255:
                return render_template('hypervisor_server_create.html', hypervisor=hypervisor, hypervisor_ip_check=True)

        if fdata['hv_server_name'] == "":
            return render_template('hypervisor_server_create.html', user=user, blank_password=True)

        result = HyperVisor.add_hypervisor(hypervisor);

        if result == 'HyperVisor Name already existed':
            return render_template('hypervisor_server_create.html', hypervisor=hypervisor, hypervisor_name_duplicate=True)

        if result == 'HyperVisor IP already existed':
            return render_template('hypervisor_server_create.html', hypervisor=hypervisor, hypervisor_ip_duplicate=True)

        return redirect(url_for('hypervisor_server_create'))


@app.route('/hypervisor/add', methods=['GET', 'POST'])
@login_required
def add_new_hyper_visor():
    if request.method == 'GET':
        return render_template('add_new_hyper_visor.html')
    if request.method == 'POST':
        # get new profile info
        hv_server_name = request.form['hv_server_name'] if 'hv_server_name' in request.form else ''
        hv_server_ip = request.form['hv_server_ip'] if 'hv_server_ip' in request.form else ''
        hv_server_api_user = request.form['hv_server_api_user'] if 'hv_server_api_user' in request.form else ''
        hv_server_api_password = request.form['hv_server_api_password'] if 'hv_server_api_password' in request.form else ''
        print("hv_server_name: {0}".format(hv_server_name))
        print("hv_server_ip: {0}".format(hv_server_ip))
        print("hv_server_api_user: {0}".format(hv_server_api_user))
        print("hv_server_api_password: {0}".format(hv_server_api_password))
        # update user profile
        hypervisor = HyperVisor(hv_server_name=hv_server_name, hv_server_ip=hv_server_ip, hv_server_api_user=hv_server_api_user, hv_server_api_password=hv_server_api_password, reload_info=True)
        hypervisor.update_hypervisor()
        
        return render_template('add_new_hyper_visor.html')

@app.route('/hypervisor/server/delete', methods=['GET', 'POST'])
@login_required
def hypervisor_delete():
    hv_server_name = request.form['hv_server_name'] if 'hv_server_name' in request.form else ''
    hv = HyperVisor()
    result = hv.delete(hv_server_name)
    return make_response(jsonify( { 'hv_server_name': 'ok', 'msg': '{0}' } ), 200).format(hv_server_name)
    if result['status'] == 'error':
        return redirect(url_for('error', code=500))

    return redirect(url_for('hypervisor'))

@app.route('/hypervisor/remove', methods=['GET', 'POST'])
@login_required
def remove_hyper_visor():
    if request.method == 'POST':
        # get new profile info
        hv_server_name = request.form['hv_server_name'] if 'hv_server_name' in request.form else ''
        hv_server_ip = request.form['hv_server_ip'] if 'hv_server_ip' in request.form else ''
...
@app.route('/hypervisor', methods=['GET', 'POST'])
@login_required
def hypervisor():
    if request.method == 'GET':
        d = HyperVisor()
        if current_user.role.name == 'Administrator':
            hypervisors_all = HyperVisor.query.all()
        else:
            return redirect(url_for('error', code=401))
        return render_template('hypervisor.html',hypervisors=hypervisors_all)
    if request.method == 'POST':
        try:
            pdata = request.data
            ...
            return make_response(jsonify( { 'data': 'ok', 'msg': '{0}' } ), 200).format(data)
            return make_response(jsonify( { 'action': 'ok', 'msg': '{0}' } ), 200).format(action)
            if jdata['action'] == 'delete_hypervisor':
                hypervisor = HyperVisor(hv_server_name=data)
                result = hypervisor.delete()
                if result:
                    return make_response(jsonify( { 'status': 'ok', 'msg': 'HyperVisor has been removed.' } ), 200)
                else:
                    return make_response(jsonify( { 'status': 'error', 'msg': 'Cannot remove HyperVisor.' } ), 500)
            else:
    ...
    if current_user.role.name == 'Administrator':
        hypervisors_all = HyperVisor.query.all()
        hypervisors_count = HyperVisor.query.count()
        return render_template('hypervisor.html', hypervisors=hypervisors_all)
    else:
        return redirect(url_for('error', code=401))
