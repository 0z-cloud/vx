...
class HyperVisor(db.Model):
    ...
    hv_server_name = db.Column(db.String(64), index=True, unique=True)
    hv_server_ip = db.Column(db.String(64), index=True, unique=True)
    hv_server_api_user = db.Column(db.String(64))
    hv_server_api_password = db.Column(db.String(64))

    def __init__(self, id=None, hv_server_name=None, hv_server_ip=None, hv_server_api_user=None, hv_server_api_password=None, reload_info=True):
        self.id = id
        self.hv_server_name = hv_server_name
        self.hv_server_ip = hv_server_ip
        self.hv_server_api_user = hv_server_api_user
        self.hv_server_api_password = hv_server_api_password
        print("hv_server_name {0}".format(hv_server_name))

        if reload_info:
            # user_info = self.get_user_info_by_id() if id else self.get_user_info_by_username()
            hv_info = self.get_hv_info_by_id() if id else self.get_hv_info_by_name()

            if hv_info:
                self.id = hv_info.id
                self.hv_server_name = hv_info.hv_server_name
                self.hv_server_ip = hv_info.hv_server_ip
                self.hv_server_api_user = hv_info.hv_server_api_user
                self.hv_server_api_password = hv_info.hv_server_api_password
    ...
    def __repr__(self):
        return '<hv_server_name %r>' % (self.hv_server_name)

    def get_hashed_password(self, hv_server_api_password=None):
        # Hash a password for the first time
        #   (Using bcrypt, the salt is saved into the hash itself)
        pw = hv_server_api_password if hv_server_api_password else self.hv_server_api_password
        return bcrypt.hashpw(pw.encode('utf-8'), bcrypt.gensalt())

    def check_password(self, hashed_password):
        # Check hased password. Useing bcrypt, the salt is saved into the hash itself
        return bcrypt.checkpw(self.hv_server_api_password.encode('utf-8'), hashed_password.encode('utf-8'))

    def get_hv_info_by_id(self):
        hv_info = HyperVisor.query.get(int(self.id))
        return hv_info

    def get_hv_info_by_name(self):
        hv_info = HyperVisor.query.filter(HyperVisor.hv_server_name == self.hv_server_name).first()
        return hv_info

    def get_hv_info_by_ip(self):
        hv_info = HyperVisor.query.filter(HyperVisor.hv_server_ip == self.hv_server_ip).first()
        return hv_info
    ...
    def is_validate(self, method):
        ...
        if hv_info:
            if hv_info.hv_server_api_password and self.check_password(hv_info.hv_server_api_password):
                logging.info('hv_server_api_password for server "%s" checked in successfully' % self.hv_server_name)
                return True
            logging.error('hv_server_api_password for server "%s" a wrong' % self.hv_server_name)
            return False
        logging.error('Unsupported validation method')
        return False
        
    def add_hypervisor(self):
        """
        Add new hypervisor witch stores in the DB
        """
        # check if username existed
        hypervisor = HyperVisor.query.filter(HyperVisor.hv_server_name == self.hv_server_name).first()
        if hypervisor:
            return 'HyperVisor Name already existed'

        # check if email existed
        hypervisor = HyperVisor.query.filter(HyperVisor.hv_server_ip == self.hv_server_ip).first()
        if hypervisor:
            return 'HyperVisor IP already existed'

        # first register user will be in Administrator role
        self.hv_server_name = self.hv_server_name
        self.hv_server_ip = self.hv_server_ip
        self.hv_server_api_user = self.hv_server_api_user
        # self.hv_server_api_password = self.get_hashed_password(self.hv_server_api_password)
        self.hv_server_api_password = self.hv_server_api_password

        db.session.add(self)
        db.session.commit()
        return True

    def update_hypervisor(self, enable_otp=None):
        """
        Update hypervisor profile
        """
        ...
        hypervisor.hv_server_name = self.hv_server_name if self.hv_server_name else hypervisor.hv_server_name
        hypervisor.hv_server_ip = self.hv_server_ip if self.hv_server_ip else hypervisor.hv_server_ip
        hypervisor.hv_server_api_user = self.hv_server_api_user if self.hv_server_api_user else hypervisor.hv_server_api_user
        hypervisor.hv_server_api_password = self.get_hashed_password(self.hv_server_api_password) if self.hv_server_api_password else hypervisor.hv_server_api_password
    ...
    def get_hypervisors(self):
        """
        Get hypervisors server
        """
        hypervisors_servers = []
        query = db.session.query(HyperVisor).filter(HyperVisor.id==self.id).filter(HyperVisor.hv_server_name==HyperVisor.hv_server_name).filter(HyperVisor.hv_server_ip==HyperVisor.hv_server_ip).all()
        for q in query:
            hypervisors_servers.append(q[2])
        return hypervisors_servers

    def delete(self):
        """
        Delete a HyperVisor Server
        """
        try:
            HyperVisor.query.filter(HyperVisor.hv_server_name == self.hv_server_name).delete()
            db.session.commit()
            return True
        except:
            db.session.rollback()
            logging.error('Cannot delete Hyper Visor %s from DB' % self.hv_server_name)
            return False

class HyperVisorServer(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    HyperVisorServerName = db.Column(db.String(64), index=True, unique=True)
    HyperVisorServerIP = db.Column(db.String(128), index=True, unique=True)
    HyperVisorServerApiUser = db.Column(db.String(64))
    HyperVisorServerApiPassword = db.Column(db.String(64))

    def __init__(self, id=None, HyperVisorServerName=None, HyperVisorServerIP=None, HyperVisorServerApiUser=None, HyperVisorServerApiPassword=None):
        self.id = id
        self.HyperVisorServerName = HyperVisorServerName
        self.HyperVisorServerIP = HyperVisorServerIP
        self.HyperVisorServerApiUser = HyperVisorServerApiUser
        self.HyperVisorServerApiPassword = HyperVisorServerApiPassword
    ...
    def __repr__(self):
        return '<HyperVisor %r>' % (self.HyperVisorServerName)

    def get_hypervisor_info_by_id(self):
        hypervisor_info = HyperVisorServer.query.get(int(self.id))
        return hypervisor_info

    def get_hypervisor_info_by_name(self):
        hypervisor_info = HyperVisorServer.query.filter(HyperVisorServer.HyperVisorServerName == self.HyperVisorServerName).first()
        return hypervisor_info

    def get_hypervisor_info_by_ip(self):
        hypervisor_info = HyperVisorServer.query.filter(HyperVisorServer.HyperVisorServerIP == self.HyperVisorServerIP).first()
        return hypervisor_info
    def add_hypervisor(self):
        """
        Add hypervisor
        ...
        hypervisor = HyperVisorServer.query.filter(HyperVisorServer.HyperVisorServerName == self.HyperVisorServerName).first()
        if not hypervisor:
            return False

        hypervisor.HyperVisorServerName = self.HyperVisorServerName if self.HyperVisorServerName else hypervisor.HyperVisorServerName
        hypervisor.HyperVisorServerIP = self.HyperVisorServerIP if self.HyperVisorServerIP else hypervisor.HyperVisorServerIP
        hypervisor.HyperVisorServerApiUser = self.HyperVisorServerApiUser if self.HyperVisorServerApiUser else hypervisor.HyperVisorServerApiUser
        hypervisor.HyperVisorServerApiPassword = self.get_hashed_password(self.HyperVisorServerApiPassword) if self.HyperVisorServerApiPassword else hypervisor.HyperVisorServerApiPassword
