...
options:
    ...
    state:
        description:
            - state of nic (present/update/absent/validate).
...
VAPP_VM_NIC_STATES = ['present', 'absent', 'update', 'validate']
...
class VappVMNIC(VcdAnsibleModule):
    ...
    def update_nic(self):
        ...
        try:
            ...
            msg = 'A nic with checked ID are not found are connected and attached to VM {0}, go to create nic'
            response['msg'] = msg
            add_nic_result = self.add_nic()
            response['add_nic_result'] = add_nic_result
            response['changed'] = True
        return response
