...
from ansible.module_utils.vapp import VApp
import ansible.module_utils.utils 
...
def vapp_vm_argument_spec():
    return dict(
        ...
        source_vm_name=dict(type='str', required=True),
...
class VappVM(VcdAnsibleModule):
    ...
    def aol_update_vm_ng(self):
        ...
        add_vms_task = self.vapp.update_vm_guest_customization_item(spec)
        self.execute_task(add_vms_task)
        response['msg'] = 'VM {} has been updated.'.format(target_vm_name)
    ...
    def update_vm(self):
        ...
        if self.params.get("cust_script"):
            self.update_vm_ng()
            response['changed'] = True
    ...
    def update_vm_ng(self):
        ...
        params = E.RecomposeVAppParams(
            state='update' if state else 'false', deploy='true' if deploy else 'false', powerOn='true' if power_on else 'false')
    ...
    def update_cust_script(self):
        ...
        vm = self.get_vm()
        update_cust_script_task = self.vapp.update_vm_guest_customization_item()
        do_deploy 
        #task = vm.update_guest_customization_section(enabled=True)
        #result = vm.get_task_monitor().wait_for_success(update_cust_script_task)
        return self.execute_task(update_cust_script_task)
        # self.result = result
        # return self.result
