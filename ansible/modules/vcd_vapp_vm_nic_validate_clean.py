...
DOCUMENTATION = '''
---
module: vcd_vapp_vm_nic_validate_clean
short_description: Validate and Operate VM NIC's states/operations in vCloud Director
version_added: "2.9.9"
description:
    - Validate and Operate VM NIC's states/operations in vCloud Director
...
- name: '[ethX] Create NIC and Attach NIC with ID to NICs Specs by Network in vCloud vApp Zone'
  vcd_vapp_vm_nic_validate_clean:
...
class VMExtNG(object):
    ...
    def add_nic(self, adapter_type, is_primary, is_connected, network_name,
                ip_address_mode, ip_address, nic_id):
...
class BooleanReturner(VcdAnsibleModule):
    def __init__(self, **kwargs):
        super(BooleanReturner, self).__init__(**kwargs)
        ...
        input_module = self
        networks_check = None
        if hasattr(input_module, 'ncls'):
            networks_check = True
        else:
            networks_check = False
        self.networks_check = networks_check
...
class VappVMNIC(VcdAnsibleModule):
    ...
    def manage_states(self):
        ...
        if len(already_in_nics_list) == 0:
            if state == "validate":
                response = self.add_nic_do_ng_return_self()
    ...
    def get_vm(self):
        ...
        return VMExtNG(self.client, resource=vapp_vm_resource)
    ...
    def add_nic_do_ng_return_self(self):
        vm = self.get_vm()
        vm_name = self.params.get('vm_name')
        nic_id = self.params.get('nic_id')
        network = self.params.get('network')
        ...
        response['z_add_nic_do_ng_return_self'] = {
    ...
    def read_nics(self):
        ...
        try:
            ...
            response_add_returned = self.add_nic_do_ng_return_self()
            ncls = vm.list_nics()
            self.ncls = ncls
...
def main():
    ...
    global uno_verification_step_response
    argument_spec = vapp_vm_nic_argument_spec()
    response = dict(msg=dict(type='str'))
    uno_verification_step_response = dict(msg=dict(type='str'))
    tre_operation_step_response = dict(msg=dict(type='str'))
