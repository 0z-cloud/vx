...
do
  ...
  redis-cli -h ${hosts[$index]} -p ${ports[$index]} cluster addslots $i > /dev/null {% endraw %}
  {% else %}{% raw %}
  redis-cli -a {% endraw %}{{ redis_cluster_secured_by_password}}{% raw %} -h ${hosts[$index]} -p ${ports[$index]} cluster addslots $i > /dev/null {% endraw %}
  {% endif %}{% raw %}
done
