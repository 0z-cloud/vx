...
if [ ! -z $linked_env_in_product ]; then
    . $ansible_dir/\!_tests/7z-lf_edge_eve_development_feel_local.sh $linked_env_in_product
else
    . $ansible_dir/\!_tests/7z-lf_edge_eve_development_feel_local.sh
fi
...
# # BUILD IMAGES INSIDE
# linked_env_in_product="production-eden-eve"
# ANSIBLE_ENVIRONMENT="production-eden-eve"

if [ ! -z $linked_env_in_product ]; then

    /bin/bash "$ansible_dir"/reference_7z-eve-cloud.sh \
    "$ANSIBLE_ENVIRONMENT" \
