#!/bin/bash
################################################################################
SCRIPT_NAME="reference_develoment.sh"
PIPELINE_NAME="Development Only"
################################################################################
inventory="$1"
product="$2"
username="$3"
...
if [ ! -z $remap_target_to_link_inventory_name ]; then
    ...
    rnd_development_run_bootstrap_and_check_infra "$typeofcloud" "$ansible_dir" "$product" "$inventory" "$ansible_datacenter";
    ...
    #
    #
    . "$ansible_dir"/scripts/libsh/development_cloud_api_gen.sh "$runpath" "$ansible_dir" "$remap_target_to_link_inventory_name"
    ...
    target_inventory_link_devops_service_zone_live "$typeofcloud" "$ansible_dir" "$product" "$inventory" "$ansible_datacenter";
